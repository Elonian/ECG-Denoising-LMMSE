...
$$
w_{\mathrm{train}}[n] = y_{\mathrm{train}}[n] - s_{\mathrm{train}}[n].
$$
...
$$
\hat{\mathbf{s}}_{\mathrm{test}} =
\mathbf{Y}_{P,\mathrm{test}}\hat{\mathbf{h}}.
$$
...
\qquad
\mu_y = \mathrm{mean}(y_{\mathrm{train}}),
\qquad
\sigma_y = \mathrm{std}(y_{\mathrm{train}}).
$$

For a normalized window $\tilde{\mathbf{y}}_P[n]$, each scalar is projected into a $d_{\mathrm{model}}$ dimensional token:
